...
def _generate_chunk_slices(parallel_coords, input_data_name):
    ...
    for xds_key in input_data_name:
        for dim, pc in parallel_coords.items():
            ...
            interp_indx = interp1d_dict[dim](
                input_data_name[xds_key][dim].values
            ).astype(int)
            ...
            for chunk_key in np.arange(
                ...
                pc_chunk_start = np.where(
                    np.array(pc["data"]) == pc["data_chunks"][chunk_key][0]
                )[0][
                    0
                ]  # pc_chunk_end
                ...
                if (
                    pc_chunk_start > interp_indx[-1] and pc_chunk_end > interp_indx[-1]
                ) or (
                    pc_chunk_start < interp_indx[0] and pc_chunk_end < interp_indx[0]
                ):
                    start_slice = None
                    end_slice = None
                else:
                    ...
                    if pc_chunk_start < interp_indx[0]:
                        ...
                        start_slice = []
                        increase_index = 0
                        while (len(start_slice) == 0) and (
                            pc_chunk_start + increase_index <= interp_indx[-1]
                        ):
                            start_slice = np.where(
                                interp_indx == pc_chunk_start + increase_index
                            )[0]
                    ...
                    if pc_chunk_end > interp_indx[-1]:
                        ...
                        while (len(end_slice) == 0) and (reduce_index < pc_chunk_end):
                            end_slice = (
                                np.where(
                                    interp_indx == pc_chunk_end - 1 - reduce_index
                                )[0]
                                + 1
                            )
...
def map(
    ...
    if input_data_type == "processing_set":
        input_data = read_processing_set(
            input_data_name, ps_sel_parms["intents"], ps_sel_parms["fields"]
        )
